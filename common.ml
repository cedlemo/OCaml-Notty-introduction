...
module List = struct
  ...
  let rec chunks n xs =
    match splitat n xs with a, [] -> [ a ] | a, b -> a :: chunks n b
...
module Images = struct
  ...
  let i5 =
    let open I in
    tile 5 1 List.(range 0 15 |> map (fun i -> pad ~t:i ~l:(i * 2) i2) |> zcat)
...
let simpleterm ~imgf ~f ~s =
  ...
  let rec go s =
    ...
    | `Resize _ -> go s
    | #Unescape.event as e -> ( match f s e with Some s -> go s | _ -> () )
  in
  go s
